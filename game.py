...
class Zombie:
    ...
    def update(self):
        ...
        self.rotation_degrees = (180 / math.pi) * - math.atan2(player.hitbox.centery - self.hitbox.centery, player.hitbox.centerx - self.hitbox.centerx)
...
class Bullet:
    def __init__(self, x, y, rotation_degrees):
        ...
        self.hitbox = pygame.Rect(self.x - self.hitRadius, self.y - self.hitRadius, 2 * self.hitRadius, 2 * self.hitRadius)
        self.speed = 4
    ...
    def move(self):
        ...
        self.hitbox = pygame.Rect(self.pos[0] - self.hitRadius, self.pos[1] - self.hitRadius, 2 * self.hitRadius, 2 * self.hitRadius)
...
def drawUI():
    ...
    coin_text = font16.render( "Coins: " + str(player.coins) + " | day: " + str(dayCount) + " | kills: " + str(player.killcount), True, YELLOW)
    ...
    if player.reloading:
        ...
        pygame.draw.rect(screen, RED, pygame.Rect(WINDOW_W * 0.85, WINDOW_H * 0.92, (100 / player.equippedGun.clipsize) * (player.equippedGun.ammo_in_clip), 10))
...
def run():
    ...
    showing_endscreen = True
    while showing_endscreen:
        for event in pygame.event.get():
                match event.type:
                    case pygame.QUIT:
                        showing_endscreen = False
